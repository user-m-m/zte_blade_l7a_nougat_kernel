...
static struct root_record kill_process_table[] = {
	...
	{"debuggerd:sig", "debuggerd", 0, 0, 0, 0},
...
static struct root_record sys_admin_table[] = {
	...
	{"adbd", "init", 1, 0, 0, 0},
	{"factorytest", "init", 1, 0, 0, 0},
	{"mount", "factorytest", 0, 0, 0, 0},
	{"update_binary", "recovery", 0, 0, 0, 0},
	{"", "", 0, 0, 0, 0},
};
...
static struct list_head record_list;
...
static void write_root_stat_emmc(void)
{
	...
	struct record_list_node *rln;
	mm_segment_t old_fs;
	char *miscdata_name;

	list_for_each_entry(rln, &record_list, node) {
		if(!root_stat_cmp(&rln->rec, &write_record)) {
			goto err_file_path;
		}
	}
	rln = (struct record_list_node *)kmalloc(sizeof(struct record_list_node), GFP_KERNEL);
	if (!rln)
		goto err_file_path;
	memcpy(&rln->rec, &write_record, sizeof(write_record));
	list_add(&rln->node, &record_list);
