...
static volatile int button_state_last = 0; 
static int current_key_code = KEY_RESERVED;
static int plug_state_last = 0; 
...
no_mic_retry:
        ...
        if((adc_mic_average < pdata->adc_threshold_3pole_detect)) {
                ...
                else if(pdata->gpio_detect_mic == gpio_detect)
...
static void reg_dump_func(struct work_struct *work)
{
	...
	int gpio_detect_mic = 0;
        ...
        ana_pmu0 = sci_adi_read(ANA_AUDCFGA_INT_BASE+ANA_PMU0); 
        ana_hdt0 = sci_adi_read(ANA_AUDCFGA_INT_BASE+ANA_HDT0); 
        ana_sts0 = sci_adi_read(ANA_AUDCFGA_INT_BASE+ANA_STS0); 

        sci_adi_write(ANA_REG_GLB_ARM_MODULE_EN, BIT_ANA_HDT_EN, BIT_ANA_HDT_EN); 
        sci_adi_write(ANA_REG_GLB_ARM_CLK_EN, BIT_CLK_AUD_HID_EN, BIT_CLK_AUD_HID_EN); 

        hid_cfg2 = sci_adi_read(ANA_HDT_INT_BASE+HID_CFG2); 
        hid_cfg3 = sci_adi_read(ANA_HDT_INT_BASE+HID_CFG3); 
        hid_cfg4 = sci_adi_read(ANA_HDT_INT_BASE+HID_CFG4); 
        hid_cfg0 = sci_adi_read(ANA_HDT_INT_BASE+HID_CFG0); 
