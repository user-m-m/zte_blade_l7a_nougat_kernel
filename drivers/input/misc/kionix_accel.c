...
#define KIONIX_KMSG_ERR	1	 
#define KIONIX_KMSG_INF	1	 
...
#define INPUT_REPORT_ABS  1
#define CAL_COUNT		5
//#define	CAL_PATH	"/data/KIONIX_ACC_CAL.bin"
...
#define KIONIX_ACCEL_WHO_AM_I_KXTJ2_1057 	0x35   
/******************************************************************************
 * Accelerometer Grouping
 *****************************************************************************/
#define KIONIX_ACCEL_GRP1	1	 
#define KIONIX_ACCEL_GRP2	2	 
#define KIONIX_ACCEL_GRP3	3	 
#define KIONIX_ACCEL_GRP4	4	 
#define KIONIX_ACCEL_GRP5	5	 
#define KIONIX_ACCEL_GRP6	6	 
#define KIONIX_ACCEL_GRP7	7	 
...
#define ACCEL_GRP7_DATA_CTRL	0x1B	
...
#define ACC_UNIT_CONVERSION(value) ((value) * GRAVITY_EARTH_1000  / (1024))
...
static int qmm_cali_x = 0, qmm_cali_y = 0, qmm_cali_z = 0;
static int qmm_x = 0, qmm_y = 0, qmm_z = 0;
...
extern char g_sensor_vendor_info[20];
extern char g_sensor_ic_info[20];
#endif

static struct proc_dir_entry * g_sensor_proc_entry=NULL;
...
struct kionix_accel_driver *g_acceld=NULL;
...
static s32 g_sensor_cali_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	int i=0;
	char buffer[12];
	int * buffer_int=NULL;
	printk(" wellcome to g_sensor_cali_write !!");
	if(12 != len){
	...
	for(i=0;i<G_SENSOR_ALL;i++){
		printk("g_sensor_cali_write [%d]:%d ",i,g_sensor_compensation[i]);
	}
	return len;
}
//struct file *, char __user *, size_t, loff_t *
static s32 g_sensor_cali_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	int i =0;
	int j =0;
	printk(" wellcome to g_sensor_cali_read !!\n");
		printk(" wellcome to g_sensor_cali_read !!\n");
	if(NULL == g_acceld){
		printk("g_acceld is NULL !!\n");
		return -1;
	}
	
	memset(g_sensor_array,0,sizeof(int)*G_SENSOR_ALL*G_SENSOR_CAIL_NUM);
	if(kionix_accel_enable(g_acceld) !=0){
		printk("can't enable sensor data!!\n");
		return -1;
	}
	...
	msleep(600);
	if(point_num==0){
		printk("no sensor data!!\n");
		return -1;
	}else{
		printk("sensor data ok!!\n");
	}// 1 clear the array 
	if(kionix_accel_disable(g_acceld) !=0){
		printk("can't disable sensor data!!\n");
		return -1;
	}
	...
	for(i=0;i<G_SENSOR_ALL;i++){
		...
		g_sensor_compensation[i]=g_sensor_sum[i]/(G_SENSOR_CAIL_NUM-(TAKE_OUT*2));
	}
	...
	g_acceld->accel_cali[g_acceld->axis_map_z]=g_sensor_compensation[2];
	for(i=0;i<G_SENSOR_ALL;i++){
		printk("end the cali g_sensor_compensation[%d]:[%d]\n",i,g_sensor_compensation[i]);
...
static void kionix_accel_grp4_report_accel_data(struct kionix_accel_driver *acceld)
{
	...
	int err;
	struct input_dev *input_dev = acceld->input_dev;   
	int direction = acceld->accel_pdata.accel_direction;
	int loop;
	int xyz[3] = {0};
	...
	{
		{
			...
			else 
			{
				...
				y = ((s16) le16_to_cpu(accel_data.accel_data_s16[acceld->axis_map_y])) >> acceld->shift;
				z = ((s16) le16_to_cpu(accel_data.accel_data_s16[acceld->axis_map_z])) >> acceld->shift;
				printk("raw--%s--p[%d] x:[%d] y:[%d] z:[%d]\n",__func__,point_num,x,y,z);
				...
				//qmm_cali_x = acceld->accel_cali[acceld->axis_map_x];
				//qmm_cali_y = acceld->accel_cali[acceld->axis_map_y];
				//qmm_cali_z = acceld->accel_cali[acceld->axis_map_z];
				acceld->accel_data[acceld->axis_map_x] = (acceld->negate_x ? -x : x) + acceld->accel_cali[acceld->axis_map_x];
				acceld->accel_data[acceld->axis_map_y] = (acceld->negate_y ? -y : y) + acceld->accel_cali[acceld->axis_map_y];
				acceld->accel_data[acceld->axis_map_z] = (acceld->negate_z ? -z : z) + acceld->accel_cali[acceld->axis_map_z];
				{
					xyz[0] = ACC_UNIT_CONVERSION(acceld->accel_data[acceld->axis_map_x]);
					xyz[1] = ACC_UNIT_CONVERSION(acceld->accel_data[acceld->axis_map_y]);
					xyz[2] = ACC_UNIT_CONVERSION(acceld->accel_data[acceld->axis_map_z]);
					 
					coordinate_data_convert(xyz, ORI);
					printk(KERN_ERR "QMM after convert %s xyz[0] = %d, xyz[1] = %d, xyz[2] = %d\n", __func__, xyz[0], xyz[1], xyz[2]);
					//input_report_abs(acceld->input_dev, ABS_X, acceld->accel_data[acceld->axis_map_x]);
					//input_report_abs(acceld->input_dev, ABS_Y, acceld->accel_data[acceld->axis_map_y]);
					//input_report_abs(acceld->input_dev, ABS_Z, acceld->accel_data[acceld->axis_map_z]);
#if INPUT_REPORT_ABS
					input_report_abs(acceld->input_dev, ABS_X, xyz[0]);
					input_report_abs(acceld->input_dev, ABS_Y, xyz[1]);
					input_report_abs(acceld->input_dev, ABS_Z, xyz[2]);
#else
					input_event(acceld->input_dev, EV_MSC, MSC_RAW, xyz[0]);
					input_event(acceld->input_dev, EV_MSC, MSC_SCAN, xyz[1]);
					input_event(acceld->input_dev, EV_MSC, MSC_GESTURE, xyz[2]);
#endif
					input_sync(acceld->input_dev);
				}
				write_unlock(&acceld->rwlock_accel_data);
...
int kionix_load_cali(struct kionix_accel_driver *);

int kionix_load_cali(struct kionix_accel_driver *acceld)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	loff_t pos;
	...
	old_fs = get_fs();
    set_fs(KERNEL_DS);
	fp = filp_open(CAL_PATH, O_RDWR, 0664);
	if(IS_ERR(fp))
	{
		printk(KERN_ERR "QMM %s open cal file failed\n", __func__);
	...
	printk(KERN_ERR "QMM %s open cal file success\n", __func__);

	 
    pos = 0;		 
	vfs_read(fp, (u8*)cal_buf, 16, &pos);
	filp_close(fp, NULL);
    set_fs(old_fs);
	
	 

	printk(KERN_ERR "Load cali, QMM %s cal_buf[0] = %d, cal_buf[1] = %d, cal_buf[2] = %d, cal_buf[3] = %d\n", __func__, cal_buf[0], cal_buf[1], cal_buf[2], cal_buf[3]);
	if((cal_buf[0] + cal_buf[1] + cal_buf[2]) == cal_buf[3])
	{
		acceld->accel_cali[acceld->axis_map_x] = cal_buf[0];
		acceld->accel_cali[acceld->axis_map_y] = cal_buf[1];
		acceld->accel_cali[acceld->axis_map_z] = cal_buf[2];
	}
	printk(KERN_ERR "QMM %s load cali data success\n", __func__);
	return 0;
}
...
static void kionix_accel_work(struct work_struct *work)
{
	...
	//KMSGINF(&acceld->client->dev, "<%s> start L%d.\n", __func__, __LINE__);
#if 0
	if(0 == kionix_cali_flag && 0 == cali_ing_flag)
	{
		printk(KERN_ERR "%s start load cali\n", __func__);
		ret = kionix_load_cali(acceld);
		if(ret >= 0)
		{
			printk(KERN_ERR "%s load cali success\n", __func__);
			kionix_cali_flag = 1;
		}
	}
#endif
...
{
	int err = 0;
    printk("QMM kionix_accel_enable\n");
...
static int kionix_accel_disable(struct kionix_accel_driver *acceld)
{
    ...
    printk("QMM kionix_accel_disable\n");
...
/******** QMM for test cali*******/
static ssize_t kionix_accel_get_cali_result(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "cali qmm %d %d %d\n", qmm_cali_x, qmm_cali_y, qmm_cali_z);
}
static ssize_t kionix_accel_get_data_result(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "data qmm %d %d %d\n", qmm_x, qmm_y, qmm_z);
}
/******** QMM for test cali*******/
...
//	mdelay(10);
	...
	err = acceld->kionix_accel_update_odr(acceld, 20);        
	if (err < 0) 
	{
		KMSGERR(&acceld->client->dev,
	...
	calibration[0] = -sum_x;
	calibration[1] = -sum_y;
	calibration[2] = -sum_z + 1024;
	sprintf(cal_buf, "%d %d %d %d", calibration[0], calibration[1], calibration[2], 
        calibration[0] + calibration[1] + calibration[2]);
	printk(KERN_ERR "Cali success! %s:%s\n", __func__, cal_buf);
	...
	{
		KMSGERR(&acceld->client->dev,
			"%s: Create calibration file fail.\n",
			__func__);
		printk(KERN_ERR "%s Create calibration file fail.\n", __func__);
		return -1;
	}
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	pos = 0;		 
	vfs_write(fp, cal_buf, strlen(cal_buf)+1, &pos);
	filp_close(fp, NULL);
	set_fs(old_fs);
...
#if 0
	xyz = (u16)cal_buf[0];
	xyz = (xyz << 16) | (u16)cal_buf[1];
	xyz = (xyz << 16) | (u16)cal_buf[2];
	printk("********** xyz = %llx\n", xyz);
						...
						const char *buf, size_t count)
{
	...
	{
		printk("QMM sscanf success\n");
		printk("x = %d, y = %d, z = %d\n", x, y, z);
		write_lock(&acceld->rwlock_accel_data);
		//obj->cali_sw[KXTJ2_1009_AXIS_X] = x;
		//obj->cali_sw[KXTJ2_1009_AXIS_Y] = y;
		//obj->cali_sw[KXTJ2_1009_AXIS_Z] = z;
		acceld->accel_cali[acceld->axis_map_x] = x;
		acceld->accel_cali[acceld->axis_map_y] = y;
		acceld->accel_cali[acceld->axis_map_z] = z;
		write_unlock(&acceld->rwlock_accel_data);
	...
	printk(KERN_ERR "QMM %s start set cali\n", __func__);
	...
	else 
	{
		...
		acceld->accel_cali[acceld->axis_map_y] = (int)calibration[1];
		acceld->accel_cali[acceld->axis_map_z] = (int)calibration[2];
		printk(KERN_ERR "QMM %s cal_buf[0] = %d, cal_buf[1] = %d, cal_buf[2] = %d\n", __func__, acceld->accel_cali[acceld->axis_map_x], acceld->accel_cali[acceld->axis_map_y], acceld->accel_cali[acceld->axis_map_z]);
		write_unlock(&acceld->rwlock_accel_data);
	}
...
static ssize_t kionix_accel_dev_name_show(struct device *dev,
		struct device_attribute *attr,
		char *buf, size_t count)
{
	return sprintf(buf, "%s\n", KIONIX_ACCEL_VENDOR_NAME);
}
static ssize_t kionix_accel_dev_name_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	return 0;
...
static ssize_t kionix_accel_get_chipinfo(struct device *dev,
				struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%s\n", KIONIX_ACCEL_VENDOR_NAME);
}

static DEVICE_ATTR(chipinfo, S_IRUGO, kionix_accel_get_chipinfo, NULL);
static DEVICE_ATTR(enable, 		S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH, kionix_accel_get_enable, kionix_accel_set_enable);
static DEVICE_ATTR(delay,   	S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH, kionix_accel_get_delay, kionix_accel_set_delay);
static DEVICE_ATTR(direct,  	S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH, kionix_accel_get_direct, kionix_accel_set_direct);
static DEVICE_ATTR(data,    	S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH, kionix_accel_get_data, NULL);
static DEVICE_ATTR(enable_cali, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH, kionix_accel_get_cali, kionix_accel_set_cali);
static DEVICE_ATTR(vendor_name, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH, kionix_accel_dev_name_show, kionix_accel_dev_name_store);
static DEVICE_ATTR(qmm_cali,    S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH, kionix_accel_get_cali_result, NULL);
static DEVICE_ATTR(qmm_data,    S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH, kionix_accel_get_data_result, NULL);
...
static struct attribute *kionix_accel_attributes[] = {
	...
	&dev_attr_data.attr,
	&dev_attr_qmm_cali.attr,
	&dev_attr_qmm_data.attr,
	NULL
};
...
static struct attribute *kionix_accel_input_attributes[] = {
    &dev_attr_enable_cali.attr,
	&dev_attr_vendor_name.attr,
	NULL
};

static struct attribute_group kionix_accel_input_attribute_group = {
	.attrs = kionix_accel_input_attributes
};
...
static int kionix_verify(struct kionix_accel_driver *acceld)
{
	...
	switch (retval) {
		...
		case KIONIX_ACCEL_WHO_AM_I_KX023:
			...
			break;
		case KIONIX_ACCEL_WHO_AM_I_KXTJ2_1057:
			#ifdef CONFIG_GET_DEVICES_VENDOR_INFO
			strcpy(g_sensor_ic_info, "KXTJ2-1057");
			#endif
			KMSGINF(&acceld->client->dev, "this accelerometer is a KXTJ2-1057.\n");
			break;
		default:
			break;
	}
...
static s32 kionix_tool_read( char *page, char **start, off_t off, int count, int *eof, void *data )
{
	return 1;
}

static s32 kionix_tool_write(struct file *filp, const char __user *buff, unsigned long len, void *data)
{
	return 1;
}
...
#ifdef CONFIG_OF
static struct kionix_accel_platform_data *kionix_accel_parse_dt(struct device *dev)
{
	...
	if (!pdata) {
		dev_err(dev, "Could not allocate struct kionix_accel_platform_data");
		return NULL;
	}
	...
	ret = of_property_read_u32(np, "accel_irq_use_drdy", &pdata->accel_irq_use_drdy);
	if(ret){
		dev_err(dev, "fail to get accel_irq_use_drdy\n");
		goto fail;
	}
	ret = of_property_read_u32(np, "accel_direction", &pdata->accel_direction);
	if(ret){
...
static int kionix_acc_misc_release(struct inode *inode, struct file *file)
{
        printk("kionix_acc_misc_release\n");
        return 0;
}
				...
				unsigned long arg)
{
	...
	case KIONIX_ACCEL_IOCTL_SET_ENABLE:
	{
		...
		printk("%s enable= %d\n",__FUNCTION__,enable);
		if(NULL==g_acceld){
			printk("kionix_acc_misc_ioctl error [NULL==g_acceld] !!");
			return -EINVAL;
		}
		if (enable && !pre_enable) {
			kionix_accel_enable(acceld);
...
printk("KIONIX_ACCEL_IOCTL_GET_ENABLE\n");
	...
	//		disable_irq(acceld->client->irq);
		printk("KIONIX_ACCEL_IOCTL_SET_DELAY %d\n",delay);
		/*
		 * Set current interval to the greater of the minimum interval or
		 * the requested interval
		 */
		acceld->poll_interval = max((unsigned int)delay, acceld->accel_pdata.min_interval);
		acceld->poll_delay = msecs_to_jiffies(acceld->poll_interval);
	...
	case KIONIX_ACCEL_IOCTL_GET_DELAY:
		if (copy_to_user(argp, &acceld->poll_interval, sizeof(acceld->poll_interval)))
			return -EINVAL;
		break;
	...
	case KIONIX_ACC_IOCTL_GET_XYZ:
		...
		xyz[0]=g_acceld->accel_data[g_acceld->axis_map_x]*98*100/1024;
		xyz[1]=g_acceld->accel_data[g_acceld->axis_map_y]*98*100/1024;
		xyz[2]=g_acceld->accel_data[g_acceld->axis_map_z]*98*100/1024;
		coordinate_data_convert(xyz, ORI);
...
static const struct file_operations kionix_acc_misc_fops = {
	...
	.release = kionix_acc_misc_release,
				 ...
				 const struct i2c_device_id *id)
{
	...
	err = i2c_smbus_read_byte_data(client, ACCEL_WHO_AM_I);
	printk("****** QMM ****** %s compatible read 0x%x\n", __func__, err);
	if(KIONIX_ACCEL_WHO_AM_I_KXTJ2_1009 != err && KIONIX_ACCEL_WHO_AM_I_KXTJ2_1057 != err)
	{
		printk("****** QMM ****** %s Not Kionix, return!\n", __func__);
		return -ENODEV;
	}
	printk("****** QMM ****** %s Use Kionix!\n", __func__);
	...
	if (err < 0) {
		KMSGERR(&acceld->client->dev, "%s: addr[0x%x] kionix_verify returned err = %d. Abort.\n", __func__, client->addr, err);
	...
	switch (err) {
		...
		case KIONIX_ACCEL_WHO_AM_I_KXCJK_1013:
		case KIONIX_ACCEL_WHO_AM_I_KXTJ2_1057:
			if(err == KIONIX_ACCEL_WHO_AM_I_KXTJ2_1009 || err == KIONIX_ACCEL_WHO_AM_I_KXTJ2_1057)
	...
	if (acceld->accel_drdy) {
		...
		{
			err = PTR_ERR(thread);
			KMSGERR("failed to create kernel thread: %d\n", err);
		}
#endif
	...
	err = sysfs_create_group(&acceld->input_dev->dev.kobj, &kionix_accel_input_attribute_group);
	if (err) {
		KMSGERR(&acceld->client->dev, "%s: sysfs_create_group(input device) returned err = %d. Abort.\n", __func__, err);
		goto err_free_irq;
	}
    ...
    err = sysfs_create_group(&kionix_acc_misc_device.this_device->kobj, &kionix_accel_attribute_group);
	if (err) {
		KMSGERR(&acceld->client->dev, "%s: sysfs_create_group(misc device) returned err = %d. Abort.\n", __func__, err);
	...
	g_sensor_proc_entry =  proc_create(G_SENSOR_CALI_PROCNAME, S_IRUGO, NULL,&g_sensor_cali_ops);
...
#if 0
static int kionix_suspend(struct i2c_client *client, pm_message_t mesg)
{
	struct kionix_accel_driver *acceld = i2c_get_clientdata(client);
...
static int kionix_resume(struct i2c_client *client)
{
	struct kionix_accel_driver *acceld = i2c_get_clientdata(client);
