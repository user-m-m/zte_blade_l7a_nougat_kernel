...
#define TARGET_PLATFORM                 PLATFORM_SPRD                    
...
#define DEVICE_CREATE_MODE              DEVICE_CREATE_BYPLATFORM             
    ...
    #define MIR3DA_INPUT_DEV_NAME       "acc"                           
#else
    #define MIR3DA_INPUT_DEV_NAME       "ZTE_ACCEL"                
#endif
#define MIR3DA_DRV_NAME                 "zte_acc"
#define MIR3DA_MISC_NAME                MIR3DA_DRV_NAME

#define GRAVITY_EARTH                   9807
#define ABSMIN_2G                       (-GRAVITY_EARTH * 2)
#define ABSMAX_2G                       (GRAVITY_EARTH * 2)
#define ACC_CONVERSION(value)           (value * GRAVITY_EARTH / 1024)
...
static ssize_t mir3da_chipinfo_show(struct device *dev,
                   struct device_attribute *attr, char *buf){
	return sprintf(buf, "%s\n", "MiraMEMS DA215");
}

static ssize_t mir3da_accel_get_cali(struct device *dev,
                   struct device_attribute *attr, char *buf){
    s8              z_dir = 1; // phone face up
    struct i2c_client *client = to_i2c_client(dev);
    struct mir3da_data *mir3da = i2c_get_clientdata(client);
...
static ssize_t mir3da_accel_set_cali(struct device *dev,
                    struct device_attribute *attr,
                    const char *buf, size_t count)
{ 
    MI_MSG("MiraMEMS DA215, driver load cali file auto.\n");
    return count;
}
...
static DEVICE_ATTR(vendor_name,     S_IRUGO,            mir3da_vendor_show,             NULL); 
static DEVICE_ATTR(enable_cali,     S_IWUGO | S_IRUGO,  mir3da_accel_get_cali,          mir3da_accel_set_cali);

 
static DEVICE_ATTR(chipinfo,        S_IRUGO,            mir3da_chipinfo_show,           NULL); 
...
static int  mir3da_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if((device_create_file(misc_mir3da.this_device, &dev_attr_chipinfo)) < 0)
    {
        MI_ERR("create attribute err.\n");
