				...
				const struct gpio_keys_button *button)
{
	...
	if (gpio_is_valid(button->gpio)) {
...
#ifdef ESD_PROTECT
		setup_timer(&bdata->timer_esd, gpio_keys_esd_timer, (unsigned long)bdata);
#endif
		...
		bdata->last_state = 0xFF; 
		bdata->is_handleisr = false;
	} else {
		if (!button->irq) {
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		struct gpio_button_data *button_data = &ddata->data[i];
		button_data->is_handleisr = true;
	}
