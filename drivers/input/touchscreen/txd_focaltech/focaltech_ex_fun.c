...
//#include "ZTE_FT6336U_L7_Longfei0x90_V16_20170331_app.i"
};
/*
/*  LD */
static unsigned char CTPM_FW_LD[] = {
//#include "ZTE_FT6336U_L7_Longfei0x90_V16_20170331_app.i"
//#include"ZTE_FT6336U_L7_Longfei0x90_V17_20170707_app.i"
 #include"ZTE_FT6336U_L7A_Lide0x71_V12_20180131_app.i"
};
/*  chuangwei */
static unsigned char CTPM_FW_CW[] = {
//#include"ZTE_FT6336U_L7_VID0x85_V15_20170906_app.i"
#include"ZTE_FT6336U_L7A_Huaxinda0x69_V13_20180131_app.i"
};
 
static unsigned char CTPM_FW_HLT[] = {
#include"ZTE_FT6336U_L7A_Helitai0x82_V13_20180420_app.i"
};
...
static unsigned char ft6336U_ctpm_VidFWid_get_from_boot(  struct i2c_client *client )
{
	u8 auc_i2c_write_buf[10];
	u8 reg_val[4] = {0};
	u32 i = 0;
	...
	for(i = 0;i < FTS_UPGRADE_LOOP; i++) 
	{
     auc_i2c_write_buf[2] = (u8)(CONFIG_VENDOR_ID_ADDR >> 8);
     auc_i2c_write_buf[3] = (u8)(CONFIG_VENDOR_ID_ADDR);
		...
		if (i_ret < 0)
		{
			printk( "Step 4: read vendor id from flash error when i2c write, i_ret = %d\n", i_ret);
			continue;
		}
	...
	printk("Step 5: reset the new FW\n");
	auc_i2c_write_buf[0] = 0x07;
	fts_i2c_Write(client, auc_i2c_write_buf, 1);
	msleep(100);	 
	return vid;     
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	u8 uc_tp_fm_ver;
	unsigned char uc_boot_vendor_id;
	int i_ret;

	fts_read_reg(client, FT_REG_FW_VER, &uc_tp_fm_ver);
	fts_read_reg(client, FT_REG_VENDOR_ID, &uc_tp_fm_vender) ;
	if ((uc_tp_fm_vender != 0x71)&& (uc_tp_fm_vender != 0x69)&& (uc_tp_fm_vender != 0x82))//0x9d=ZhengHai;  0x79 = longfei TP;0X80=Each TP;0X3b=BIEL TP;
	{
		uc_boot_vendor_id = ft6336U_ctpm_VidFWid_get_from_boot(client);
		printk("[FTS] uc_boot_vendor_id= 0x%x!\n", uc_boot_vendor_id);
		if ((uc_boot_vendor_id == 0x71) || (uc_boot_vendor_id == 0x69)|| (uc_boot_vendor_id == 0x82))
		{
			uc_tp_fm_ver = 0; 
			uc_tp_fm_vender = uc_boot_vendor_id;
		}
		else
		{
	...
	uc_host_fm_ver = fts_ctpm_get_i_file_ver();
       ...
       printk("[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x,uc_tp_fm_vender = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver,uc_tp_fm_vender);
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	{
             if( uc_tp_fm_vender == 0x71){
                 pbt_buf = CTPM_FW_LD;
		   count = sizeof(CTPM_FW_LD);
             	}
	      if(uc_tp_fm_vender == 0x69){
                 pbt_buf = CTPM_FW_CW;
		   count = sizeof(CTPM_FW_CW);
             	}
		if(uc_tp_fm_vender == 0x82){
                 pbt_buf = CTPM_FW_HLT;
		   count = sizeof(CTPM_FW_HLT);
             	}
        printk("count = 0x%x,fw_lenth = 0x%x/n",count,sizeof(CTPM_FW_LD));
		i_ret = fts_6x36_ctpm_fw_upgrade(client, pbt_buf, count);
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",
					__func__);
		return i_ret;
	...
	*/
	...
	/*if (1){
		...
		i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, count);
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",
					__func__);
...
{
	u16 ui_sz,ui_sz1;
	ui_sz = sizeof(CTPM_FW_LD);
	ui_sz1 = sizeof(CTPM_FW_CW);
	if (ui_sz > 2 || ui_sz1 > 2)
	{
	    if(fts_updateinfo_curr.CHIP_ID==0x64 && uc_tp_fm_vender == 0x71)
                return CTPM_FW_LD[0x10a];
	    if(fts_updateinfo_curr.CHIP_ID==0x64 && uc_tp_fm_vender == 0x69)
                return CTPM_FW_CW[0x10a];
	    if(fts_updateinfo_curr.CHIP_ID==0x64 && uc_tp_fm_vender == 0x82)
                return CTPM_FW_HLT[0x10a];
	}
...
u8 fts_ctpm_get_i_file_vendor(void)
{
	...
	{
	    if(fts_updateinfo_curr.CHIP_ID==0x64 && uc_tp_fm_vender == 0x71)
                return CTPM_FW_LD[0x108];
	    if(fts_updateinfo_curr.CHIP_ID==0x64 && uc_tp_fm_vender == 0x69)
                return CTPM_FW_CW[0x108];
	    if(fts_updateinfo_curr.CHIP_ID==0x64 && uc_tp_fm_vender == 0x82)
                return CTPM_FW_HLT[0x108];
	}
			  ...
			  u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if(fts_updateinfo_curr.CHIP_ID==0x64 || fts_updateinfo_curr.CHIP_ID==0x06 ) 
			fts_write_reg(client, 0xbc, FT_UPGRADE_AA);
		else
			fts_write_reg(client, 0xfc, FT_UPGRADE_AA);
		msleep(fts_updateinfo_curr.delay_aa);
