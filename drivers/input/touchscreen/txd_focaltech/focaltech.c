...
#define ENTER pr_info(KERN_INFO "[FT53X6_pr_info] func: %s  line: %04d\n", __func__, __LINE__);
#define PRINT_pr_info(x...)  pr_info(KERN_INFO "[FT53X6_pr_info] " x)
#define PRINT_INFO(x...)  pr_info(KERN_INFO "[FT53X6_INFO] " x)
#define PRINT_WARN(x...)  pr_info(KERN_INFO "[FT53X6_WARN] " x)
#define PRINT_ERR(format,x...)  pr_info(KERN_ERR "[FT53X6_ERR] func: %s  line: %04d  info: " format, __func__, __LINE__, ## x)
#else
#define ENTER
 ...
 struct Upgrade_Info fts_updateinfo[] =
{
	...
	{0x64,"FT6x36",TPD_MAX_POINTS_2,AUTO_CLB_NONEED,100, 30, 0x79, 0x1c, 10, 2000},
...
extern fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client);
...
static ssize_t virtual_keys_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	return sprintf(buf,"%s:%s:%d:%d:%d:%d:%s:%s:%d:%d:%d:%d:%s:%s:%d:%d:%d:%d\n"
		...
		,__stringify(EV_KEY), __stringify(KEY_APPSELECT/*KEY_MENU*/),pdata ->virtualkeys[8],pdata ->virtualkeys[9],pdata ->virtualkeys[10],pdata ->virtualkeys[11]);
	...
	return sprintf(buf,"%s:%s:%d:%d:%d:%d:%s:%s:%d:%d:%d:%d:%s:%s:%d:%d:%d:%d\n"
		,__stringify(EV_KEY), __stringify(KEY_BACK),pdata ->virtualkeys[0],pdata ->virtualkeys[1],pdata ->virtualkeys[2],pdata ->virtualkeys[3]
		,__stringify(EV_KEY), __stringify(KEY_HOMEPAGE),pdata ->virtualkeys[4],pdata ->virtualkeys[5],pdata ->virtualkeys[6],pdata ->virtualkeys[7]
		,__stringify(EV_KEY), __stringify(KEY_MENU),pdata ->virtualkeys[8],pdata ->virtualkeys[9],pdata ->virtualkeys[10],pdata ->virtualkeys[11]);
...
{
	unsigned char ver;
	ft5x0x_read_reg(FT_REG_VENDOR_ID, &ver);
	return(ver);
}
...
static void check_gesture(int gesture_id)
{
	...
	case GESTURE_A:	
			...
			input_report_key(ft5x0x_ts->input_dev, KEY_POWER, 0);
			input_sync(ft5x0x_ts->input_dev);		    
			input_report_key(ft5x0x_ts->input_dev, KEY_A, 1);
			input_sync(ft5x0x_ts->input_dev);
			input_report_key(ft5x0x_ts->input_dev, KEY_A, 0);
			input_sync(ft5x0x_ts->input_dev);
			break;
		default:
...

{
	...
	if(IS_ERR(fp)){
		pr_info("wangcq327 --- touchpanle Open file fail error!!\n");
	}else{
...
static int tpd_enable_ps(int enable)
{
	u8 state;
	int ret = -1;
         pr_info("swordking in tpd_enable_ps");
	i2c_smbus_read_i2c_block_data(this_client, 0xB0, 1, &state);
...
static void ft5x0x_ts_suspend(struct early_suspend *handler)
{
	...
	if(IS_ERR(fp)){
		pr_info("wangcq327 --- touchpanle Open file fail !!\n");
...
static void ft5x0x_ts_hw_init(struct ft5x0x_ts_data *ft5x0x_ts)
{
	...
	pr_info("[FST] %s  lgd regulator_enable reg_vdd--\n",__func__);
	msleep(100);
	ft5x0x_ts_reset();
	msleep(200);
}
...
int fts_ctpm_auto_download(struct i2c_client *client)
{
	...
	fts_read_reg(client, FT_REG_VENDOR_ID,&uc_tp_vendor_id);
	pr_info("  yedongyue wangcq327 --- uc_tp_vendor_id == %d\n",uc_tp_vendor_id);
	pr_info("wangcq327 --- ID_pin_vol == %d \n",Check_TP_ID());
	//TP_VOL = 1;
	if(TP_VOL == 1){
			fts_read_reg(client, FT_REG_FW_VER, &uc_tp_fm_ver);
			uc_host_fm_ver =CTPM_MAIN_FW1[0x90a];
			...
			fts_read_reg(client, FT_REG_FW_VER, &uc_tp_fm_ver);
			uc_host_fm_ver =CTPM_MAIN_FW2[0x90a];
...
static ssize_t ft5x05_proc_info_read(struct file *file, char __user *buf, size_t count, loff_t *off)
{
	int ret;
	char fw_id;
	char *fw_name;
	...
	if(fw_id == 0x71)
		...
		fw_name = "not support";
	ret = sprintf(buf, "Ic:Focal-3327,Module:%s,Vendor_Id:0x%x,Fw_Version: 0x%x\n",fw_name,fw_id,ft5x0x_read_fw_ver());
	*off += ret;

	return ret;
}

static int ft5x05_create_proc_info()
{
	#define TP_INFO_NAME "tpinfo"
	static const struct file_operations tpinfo_ops = {
		.read = ft5x05_proc_info_read,
	};
	
	proc_create(TP_INFO_NAME, 0444, NULL, &tpinfo_ops);
}
