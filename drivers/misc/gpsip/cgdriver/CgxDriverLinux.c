...
#ifdef CONFIG_CGX_DMA_IRQ_MODE
#include <soc/sprd/dma.h>

extern struct sci_dma_cfg *cfg_list;
extern int node_size;
extern int cg_GpsDma_channel;
extern int g_index;
#endif
...
#ifndef CONFIG_CGX_DMA_IRQ_MODE
void CCgCpuDmaHandle(int irq,void* dev_id)
{
	CgxDriverDataReadyInterruptHandler(&DrvContext, &DrvContext.state);
}
#else
void CCgCpuDmaHandle(void *dma_ch, void* dev_id)
{
	int ret = 0;

	CgxDriverDataReadyInterruptHandler(&DrvContext, &DrvContext.state);

	if (g_index < node_size) {
		ret = sci_dma_config(cg_GpsDma_channel, &cfg_list[g_index++], 1, NULL);
		if (ret < 0)
			printk(KERN_ERR "CGX: dma config failed %d!\n", ret);

		sci_dma_register_irqhandle(cg_GpsDma_channel, TRANS_DONE, CCgCpuDmaHandle, NULL);
		sci_dma_start(cg_GpsDma_channel, DMA_GPS);
	} else
		g_index = 0;
}
#endif
