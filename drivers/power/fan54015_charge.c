...
#endif
extern uint32_t charging_call_flag; 
...
BYTE sprdchg_fan54015_cur2reg(int cur)
{
	...
	if((cur > 911) && (cur <= 1032))
		reg_val = 3;
	if((cur > 1032) && (cur <= 1154))
		reg_val = 4;
	if((cur > 1154) && (cur <= 1275))
		reg_val = 5;
	if((cur > 1275) && (cur <= 1396))
		reg_val = 6;
	if(cur > 1450)
		reg_val = 7;
	return reg_val;
...
int sprdchg_fan54015_get_charge_status(void)
{
	...
	if (chg_status == CHG_DONE && cnt < 6) {
		chg_status = CHG_CHGING;
		cnt++;
	} else {
		cnt = 0;
	}
 ...
 void sprdchg_fan54015_start_chg(int cur)
{
	...
	//if(vendor_id == VENDOR_FAN54015){ 
		...
		SPRDCHG_54015_DEBUG("sprdchg_fan54015_start_chg, VENDOR_FAN54015 reg_val=%d\n",reg_val);
	//}
	/*if(vendor_id == VENDOR_PSC5415A){
		reg_val = sprdchg_psc5415a_cur2reg(cur);
		SPRDCHG_54015_DEBUG("sprdchg_fan54015_start_chg, VENDOR_PSC5415A reg_val=%d\n",reg_val);
	}*/ 
...
int sprdchg_fan54015_get_chgcur_step(int cur, int increase)
{
	...
	//}
	/*if(vendor_id == VENDOR_PSC5415A){
		new_cur = sprdchg_psc5415a_reg2cur(reg_val);
	}*/ 
...
void sprdchg_fan54015_init(struct sprd_battery_platform_data * pdata)
{
	...
	fan54015_init();
	sprdchg_fan54015_set_vindpm(4520);
