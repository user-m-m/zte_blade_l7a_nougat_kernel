...
#if defined(CONFIG_ADIE_SC2723S) ||defined(CONFIG_ADIE_SC2723)
	if (pbat_data->chg_end_vol_pure < 4300) {	 
...
#endif
#if !(defined(CONFIG_ARCH_SCX35L64)||defined(CONFIG_ARCH_SCX35LT8))  
	sci_adi_write((ANA_CTL_EIC_BASE + 0x50), 1, (0xFFF));	 
...
#define SAMPLE_NUM  15
	...
	if (pbat_data->temp_support) {
		...
		struct adc_sample_data data = {
			...
			.channel_type = 0,	 
			.hw_channel_delay = 0,	 
			.scale = pbat_data->temp_adc_scale, 
			.pbuf = &adc_val[0],
			.sample_num = cnt,
			.sample_bits = 1,
			.sample_speed = 0,	 
 ...
 */
#define NTC_EXT_VDD_VOL		1800		 
#define NTC_R_SERIAL		47LL			 
#define NTC_R_PARALLEL		95LL	 
#define NTC_R_COMP			30			 
static uint64_t sprdchg_cal_ntc_resistance(int adcvol)
{
	...
	uint64_t mod = 0;	
	int bat_cur = sprdfgu_read_batcurrent();
	int vol_comp = bat_cur * NTC_R_COMP / 1000;		 
	...
	else
	{
		...
		x = NTC_R_SERIAL * (adcvol - vol_comp) * 1000;
		y = NTC_EXT_VDD_VOL - adcvol;
		 
		mod = do_div(x, y);
		r_ntc = x;		
	}
	printk("sprdchg: [%s]r_comp:%d, bat_cur:%d, vol_comp:%d, r_ntc=%d, adcvol=%d!\n", 
				__func__, NTC_R_COMP, bat_cur, vol_comp, r_ntc, adcvol);
	return r_ntc;
}
#endif
...
int sprdchg_read_temp(void)
{
	if (pbat_data->temp_support) {
		...
		if (pbat_data->temp_table_mode) {
			...
			printk("sprdchg: sprdchg_read_temp The calculated NTC resistance is %d ohm!\n", val);
...
static uint32_t _sprdchg_read_chg_current(void)
{
	...
	if (isense > vbat) {
		uint32_t temp = ((isense - vbat) * 1000) / 68;	 
