...
void fan54015_reg_dump(void);
static int fan54015_write_reg(int reg, u8 val)
{
	int ret;
	SPRD_EX_DEBUG("fan54015_write_reg reg = %d val = 0x%x\n",reg,val);
...
void fan54015_reg_dump(void)
{
	...
	for(i = 0;  i < 7;  i++){
		fan54015_read_reg(i, &buf[i]);
	}
	
	printk(KERN_WARNING "bat_log_reg: 0=0x%x 1=0x%x 2=0x%x 3=0x%x 4=0x%x 5=0x%x 6=0x%x 16=0x%x\n", 
			buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7]);
}
EXPORT_SYMBOL_GPL(fan54015_reg_dump);
...
void fan54015_termina_vol_set(BYTE reg_val)
{
	...
	SPRD_EX_DEBUG("lgd fan54015_termina_vol_set() val = %d\n", reg_val);
}
...
void fan54015_init(void)
{
	...
	fan54015_sw_reset();
		...
		fan54015_set_value(FAN5405_REG_CONTROL0, FAN5405_EN_STAT, FAN5405_EN_STAT_SHIFT, DISSTAT);
		printk("fan54105: release led control to AP for fan54105\n");
	
	fan54015_set_value(FAN5405_REG_CONTROL1, FAN5405_VLOWV, FAN5405_VLOWV_SHIFT,VLOWV3P4);   
	fan54015_set_value(FAN5405_REG_SP_CHARGER, FAN5405_IO_LEVEL,FAN5405_IO_LEVEL_SHIFT, ENIOLEVEL);   
	for(i = 0;  i < 15;  i++){
...
void fan54015_stop_charging(void)
{
	 ...
	 printk("stop charge,fan5405_set_ce to disable\n");
}
...
{
	if(reg_val == 0){
		fan54015_set_value(FAN5405_REG_CONTROL1, FAN5405_IINLIM, FAN5405_IINLIM_SHIFT,IINLIM500);  
	}else{
		fan54015_set_value(FAN5405_REG_CONTROL1, FAN5405_IINLIM, FAN5405_IINLIM_SHIFT,NOLIMIT);  
	}
...
void fan54015_shutdown(struct i2c_client *client)
{
	 

		fan54015_set_value(FAN5405_REG_CONTROL0, FAN5405_EN_STAT, FAN5405_EN_STAT_SHIFT, ENSTAT);
    	SPRD_EX_DEBUG("@@@@@@@fan54015_shutdown, release control to CHARGER IC\n");
