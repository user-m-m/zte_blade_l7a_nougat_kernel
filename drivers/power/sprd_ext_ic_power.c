...
static int jeita_debug_enable = 0;
static int last_temp = 200;
static int thermal_ctrl_chg_cur = 0;
...
extern void fan54015_reg_dump(void);
static unsigned long  adjust_chg_flag = 0;
uint32_t charging_call_flag; 
...
void sprdbat_thermal_ctrl_chgcurrent(int state)
{
	...
	SPRDBAT_DEBUG("thermal_ctrl_chgcurrent chgcur=%d,avg_cur=%d,state_cur = %d\n",
...
void sprdbat_chgcurrent_adjust(int state)
{
	...
	if(delta > 0){
		...
		if (sprdbat_data->bat_info.chg_stop_flags == SPRDBAT_CHG_END_NONE_BIT)
			sprd_ext_ic_op->charge_start_ext(state_cur);
	}else if(sprd_ext_ic_op->get_chgcur_step != NULL){
		cur_step = sprd_ext_ic_op->get_chgcur_step(chg_cur,0);
		state_cur = chg_cur-cur_step;
	}else{
		for(i = 1; i <= 20; i++){
...
static void sprdbat_auto_switch_cur(void)
{
	...
	if((avg_cur < 0) && (chg_cur < sprdbat_data->chg_cur_adjust_max)){
		if(sprd_ext_ic_op->get_chgcur_step != NULL){
			cur_step = sprd_ext_ic_op->get_chgcur_step(chg_cur,1);
		}else{
			cur_step = CUR_ADJUST_OFFSET;
		}
		chg_cur += cur_step;
		if(chg_cur > sprdbat_data->chg_cur_adjust_max)
			chg_cur = sprdbat_data->chg_cur_adjust_max;
		SPRDBAT_DEBUG("sprdbat_auto_increase_cur need ++chg_cur %d =>%d\n",
			chg_cur,chg_cur+ cur_step);
#ifdef CHG_CUR_JEITA
		if(sprdbat_data->last_temp_status == 0 ||
		sprdbat_data->last_temp_status >= sprdbat_data->pdata->jeita_tab_size){
	...
	if((adjust_chg_flag > 0)&& (chg_cur > sprdbat_data->chg_cur_adjust_min)){
		if(sprd_ext_ic_op->get_chgcur_step != NULL){
			cur_step = sprd_ext_ic_op->get_chgcur_step(chg_cur,0);
		}else{
			cur_step = CUR_ADJUST_OFFSET;
		}
		if(avg_cur > (cur_step + 50)){
...
static void sprdbat_temp_monitor(void)
{
	...
	if(cur_temp_status > sprdbat_data->last_temp_status){
		...
		if(sprdbat_data->bat_info.cur_temp <=
			sprdbat_data->pdata->jeita_tab[sprdbat_data->last_temp_status -1].w){
...
static struct device_attribute sprd_caliberate[] = {
	...
	SPRDBAT_CALIBERATE_ATTR_RW(charging_call),   
					...
					const char *buf, size_t count)
{
	...
	case CHG_COOL_STATE:
		...
		SPRDBAT_DEBUG("thermal_ctrl_chg_cur=%d\n",thermal_ctrl_chg_cur);
		if (thermal_ctrl_chg_cur)
			sprdbat_thermal_ctrl_chgcurrent(set_value);
		else
    ...
    case CHARGING_CALL:
        if(1 == set_value) {
            SPRDBAT_DEBUG("OHH,CHARGING_CALL: 1, charger current set to 500mA\n");
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	case BATTERY_NOW_CURRENT:
		if (sprdbat_data->bat_info.module_state ==
			...
			now_current = sprdfgu_read_batcurrent();
			
			i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
				       (now_current > 0) ? now_current : 0);
		} else {
...
static int sprdbat_start_charge(void)
{
	...
	sprdbat_data->last_temp_status =
		sprdbat_get_status_from_tab(sprdbat_data->pdata->jeita_tab_size,
								sprdbat_data->pdata->jeita_tab, jeita_debug);
...
static void sprdbat_change_module_state(uint32_t event)
{
	...
	case SPRDBAT_VBAT_OVP_RESTART_E:
		_sprdbat_clear_stopflags(SPRDBAT_CHG_END_BAT_OVP_BIT);
		break;
...
#endif
	queue_delayed_work(sprdbat_data->monitor_wqueue,
			   &sprdbat_data->sprdbat_charge_work, msecs_to_jiffies(2000));
	return 0;
}
...
static void sprdbat_update_capacty(void)
{
	...
	case POWER_SUPPLY_STATUS_FULL:
		...
		if (fgu_capacity != 100) {
			fgu_capacity = 100;
			SPRDBAT_DEBUG("fgu_capacity is force to 100\n");
		}
		if (sprdbat_data->bat_info.vbat_vol <=
		    (sprdbat_data->pdata->soft_vbat_uvlo - SPRDBAT_SHUTDOWN_OFSSET)) {
			fgu_capacity = 0;
...
static void sprdbat_call_charging(void)
{
	static unsigned int before_cur = 0;
	static unsigned int flag = 1;
	
	 
	if (flag == 0 && (!charging_call_flag)) {
		if (sprdbat_data->bat_info.ac_online) {
			sprd_ext_ic_op->charge_start_ext(before_cur);
			SPRDBAT_DEBUG("OHH, sprdbat_call_charging, ac_online, release to before_cur=%d\n", before_cur);
		} else if (sprdbat_data->bat_info.usb_online) {
			sprd_ext_ic_op->charge_start_ext(550);
			SPRDBAT_DEBUG("OHH, sprdbat_call_charging, usb_online, release to 550\n");
		}
		flag = 1;
		
	} else if (flag == 1 && charging_call_flag){//set limit currents
		if (sprdbat_data->bat_info.ac_online || sprdbat_data->bat_info.usb_online) {
			before_cur = sprd_ext_ic_op->get_charge_cur_ext();
			SPRDBAT_DEBUG("OHH, sprdbat_call_charging, set limit current, and backup old val = %d\n", before_cur);
			sprd_ext_ic_op->charge_start_ext(550);
...
#define PDATA_LOG(format, arg...) printk("sprdbat pdata: " format, ## arg)
	...
	for (i = 0; i < pdata->jeita_tab_size; i++) {
		PDATA_LOG("jeita_tab i=%d,w:%d x:%d,y:%d:z:%d\n", i,pdata->jeita_tab[i].w,
			pdata->jeita_tab[i].x,pdata->jeita_tab[i].y,pdata->jeita_tab[i].z);
	}
}
	...
	for (i = 0; i < pdata->jeita_tab_size; i++) {
		ret = of_property_read_u32_index(np, "jeita-temp-tab", i,
						 (u32 *) (&pdata->jeita_tab[i].
							  x));
		ret = of_property_read_u32_index(np, "jeita-temp-recovery-tab", i,
						 (u32 *) (&pdata->jeita_tab[i].
							  w));
		ret =
