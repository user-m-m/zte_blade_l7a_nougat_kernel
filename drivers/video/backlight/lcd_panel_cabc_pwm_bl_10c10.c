...
static int brightness_level_cali(int level)
{
    ...
    if (level <= 153) {
        level_new = level * 100 / 153;  //basic num is 153
    } else {
        level_new = 155 * (100) / (255 - 153) * (level - 153) / (100) + 100;    //155 = 255 - 100, (100): just for sample cal
    }
	
	if (level_new == 0 && level != 0) {
		level_new = 1;
	}
	//if (level_new < 6 && level_new > 0) {
	//	level_new = 6;
	//}
...
static int lcd_panel_cabc_pwm_backlight_update_status(struct backlight_device *bd)
{
	...
	level_new = brightness_level_cali(tune_level);
	BLDBG("[BACKLIGHT] lcd_panel_cabc_pwm_backlight_update_status ==> brightness_level_cali : %d\n", level_new);
#if defined(CONFIG_FB_LCD_OLED_BACKLIGHT)
        sprdfb_panel_update_brightness(bd); //OLED control
#else
        backlight_control(level_new);
#endif
