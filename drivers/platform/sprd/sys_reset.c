...
void sprd_set_reboot_mode(const char *cmd)
{
	...
	unsigned int reboot_mode_flag = 0;       
	if(cmd)
		printk("sprd_set_reboot_mode:cmd=%s\n",cmd);
		
	reboot_mode_flag = sci_adi_read(ANA_RST_STATUS);      
	dump_flag = reboot_mode_flag & HWRST_STATUS_SYSDUMP;
	pr_emerg("sprd_set_reboot_mode: reboot mode is 0x%x and dump_flagis 0x%x\n", reboot_mode_flag, dump_flag);		

	if (cmd && !(strncmp(cmd, "recovery", 8))) {
		reboot_mode_flag = HWRST_STATUS_RECOVERY;
		if (dump_flag)
			reboot_mode_flag |= HWRST_STATUS_SYSDUMP;
		pr_emerg("sprd_set_reboot_mode: recovery & reboot mode is 0x%x!\n", reboot_mode_flag);
		sci_adi_raw_write(ANA_RST_STATUS, reboot_mode_flag);	
//		sci_adi_raw_write(ANA_RST_STATUS, HWRST_STATUS_RECOVERY);
	} else if (cmd && !strncmp(cmd, "alarm", 5)) {
		reboot_mode_flag = HWRST_STATUS_ALARM;
		if (dump_flag)
			reboot_mode_flag |= HWRST_STATUS_SYSDUMP;
		pr_emerg("sprd_set_reboot_mode: alarm & reboot mode is 0x%x!\n", reboot_mode_flag);
		sci_adi_raw_write(ANA_RST_STATUS, reboot_mode_flag);	
...
//		sci_adi_raw_write(ANA_RST_STATUS, HWRST_STATUS_FASTBOOT);
		...
		if (dump_flag)
			reboot_mode_flag |= HWRST_STATUS_SYSDUMP;
		pr_emerg("sprd_set_reboot_mode: panic & reboot mode is 0x%x!\n", reboot_mode_flag);
		sci_adi_raw_write(ANA_RST_STATUS, reboot_mode_flag);	
//		sci_adi_raw_write(ANA_RST_STATUS, HWRST_STATUS_PANIC);
		...
		if (dump_flag)
			reboot_mode_flag |= HWRST_STATUS_SYSDUMP;
		pr_emerg("sprd_set_reboot_mode: special & reboot mode is 0x%x!\n", reboot_mode_flag);
		sci_adi_raw_write(ANA_RST_STATUS, reboot_mode_flag);	
...
//		sci_adi_raw_write(ANA_RST_STATUS, HWRST_STATUS_IQMODE);
		...
		reboot_mode_flag |= HWRST_STATUS_SYSDUMP;
		pr_emerg("sprd_set_reboot_mode: endump & reboot mode is 0x%x!\n", reboot_mode_flag);
		sci_adi_raw_write(ANA_RST_STATUS, reboot_mode_flag);
	} else if (cmd && !strncmp(cmd, "dumpdisable", 11)) {
		pr_emerg("sprd_set_reboot_mode: disable sysdump!\n");
		reboot_mode_flag &= ~(HWRST_STATUS_SYSDUMP);
		pr_emerg("sprd_set_reboot_mode: disdump & reboot mode is 0x%x!\n", reboot_mode_flag);
		sci_adi_raw_write(ANA_RST_STATUS, reboot_mode_flag);
//		sci_adi_raw_write(ANA_RST_STATUS, HWRST_STATUS_SPRDISK);
		...
		if (dump_flag)
			reboot_mode_flag |= HWRST_STATUS_SYSDUMP;
		pr_emerg("sprd_set_reboot_mode: normal & reboot mode is 0x%x!\n", reboot_mode_flag);
		sci_adi_raw_write(ANA_RST_STATUS, reboot_mode_flag);	
//		sci_adi_raw_write(ANA_RST_STATUS, HWRST_STATUS_NORMAL);
		...
		if (dump_flag)
			reboot_mode_flag |= HWRST_STATUS_SYSDUMP;
		pr_emerg("sprd_set_reboot_mode: ex-special & reboot mode is 0x%x!\n", reboot_mode_flag);
		sci_adi_raw_write(ANA_RST_STATUS, reboot_mode_flag);	
//		sci_adi_raw_write(ANA_RST_STATUS, HWRST_STATUS_SPECIAL);
