...
/* [Kernel_20/23] */
static int32_t isp_k_raw_afm_type1_get_statistic(struct isp_buf_node *node, struct isp_k_private *isp_private)
{
	int32_t ret = 0;
	int i = 0;
	int max_item = ISP_AFM_WIN_NUM_V1;
	...
	item = (struct isp_raw_afm_statistic *)(node->k_addr);
	memset(item, 0, sizeof(struct isp_raw_afm_statistic));
	printk("[3A Porting]isp_k_raw_afm_type1_get_statistic: After memset, node->k_addr=0x%x.\n", node->k_addr);	
	for (i = 0; i < max_item; i++) {
		item->val[i] = REG_RD((ISP_RGB_AFM_TYPE1_VAL00 + 4*i));
		printk("[3A Porting]isp_k_raw_afm_type1_get_statistic: In REG_RD for loop, item->val[i] =0x%x.\n", item->val[i]);	
	}

    isp_buf_queue_write(&isp_private->af_type1_user_queue, node);
	printk("[3A Porting]isp_k_raw_afm_type1_get_statistic:  write finished.\n");	
	return ret;
}

static int32_t isp_k_raw_afm_type2_get_statistic(struct isp_buf_node *node, struct isp_k_private *isp_private)
{
	int32_t ret = 0;
	int i = 0;
	int max_item = ISP_AFM_WIN_NUM_V1;
    ...
    isp_buf_queue_write(&isp_private->af_type2_user_queue, node);
	printk("[3A Porting]isp_k_raw_afm_type2_get_statistic: node->k_addr = 0x%x.\n", node->k_addr);
	return ret;
}

int isp_k_rawafm_type1_thread_loop(void *arg)
{
	struct isp_k_private *isp_private = (struct isp_k_private *)arg;

	if (isp_private == NULL) {
		printk("isp_k_rawafm_type1_thread_loop, isp_private is NULL \n");
		return -1;
	}
	while (1) {
		if (0 == down_interruptible(&isp_private->rawaf_type1_thread_sem)) {
			if (isp_private->is_rawaf_type1_thread_stop) {
				printk("isp_k_rawafm_type1_thread_loop stop \n");
				break;
			}
			isp_k_raw_afm_type1_get_statistic(&isp_private->rawaf_type1_cur_node, isp_private);
			up(&isp_private->rawaf_type1_schedule_sem);
			isp_private->is_rawaf_type1_schedule = 0;
			printk("[3A Porting]isp_k_rawafm_type1_thread_loop wait sem succeed!\n");
		} else {
			printk("isp_k_rawafm_type1_thread_loop wait sem failed!\n");
			break;
		}
	}
	isp_private->is_rawaf_type1_thread_stop = 0;

	return 0;
}

int isp_k_rawafm_type2_thread_loop(void *arg)
{
	struct isp_k_private *isp_private = (struct isp_k_private *)arg;

	if (isp_private == NULL) {
		printk("isp_k_rawafm_type2_thread_loop, isp_private is NULL \n");
		return -1;
	}
	while (1) {
		if (0 == down_interruptible(&isp_private->rawaf_type2_thread_sem)) {
			if (isp_private->is_rawaf_type2_thread_stop) {
				printk("isp_k_rawafm_type2_thread_loop stop \n");
				break;
			}
			isp_k_raw_afm_type2_get_statistic(&isp_private->rawaf_type2_cur_node, isp_private);
			up(&isp_private->rawaf_type2_schedule_sem);
			isp_private->is_rawaf_type2_schedule = 0;
			printk("[3A Porting]isp_k_rawafm_type2_thread_loop wait sem succeed!\n");
		} else {
			printk("isp_k_rawafm_type2_thread_loop wait sem failed!\n");
			break;
		}
	}
	isp_private->is_rawaf_type2_thread_stop = 0;

	return 0;
}

int32_t isp_k_rawafm_switch_bq_buf(struct isp_k_private *isp_private)
{
	int32_t ret = 0;
	struct isp_buf_node node;
	//int32_t ret1 = isp_buf_queue_read(&isp_private->af_type1_irq_queue, &node);
	
	if (isp_private->is_rawaf_type1_schedule || isp_buf_queue_read(&isp_private->af_type1_irq_queue, &node)) {
		ret = -1;
	} else {
		isp_private->rawaf_type1_cur_node = node;
		up(&isp_private->rawaf_type1_thread_sem);
		isp_private->is_rawaf_type1_schedule = 1;
	}

	return ret;
}

int32_t isp_k_rawafm_enqueue_buf(struct isp_k_private *isp_private, struct isp_buf_node *node)
{
	int32_t ret = 0;

		ret = isp_buf_queue_write((struct isp_buf_queue *)&isp_private->af_type1_irq_queue, node);
		printk("[3A Porting]isp_k_rawafm_enqueue_buf:TYPE1 ret=0x%x\n", ret);
     	return ret;
}

int32_t isp_k_rawafm_dequeue_buf(struct isp_k_private *isp_private, struct isp_buf_node *node)
{
	int32_t ret = 0;

	    ret = down_timeout(&isp_private->rawaf_type1_schedule_sem, ISP_RAWAFM_TIMEOUT); 
	    if (ret) {
		    printk("isp_k_rawafm_type1_dequeue_buf wait sem failed\n");
		    return -1;
		}	
	    isp_buf_queue_read(&isp_private->af_type1_user_queue, node);
		printk("[3A Porting]isp_k_rawafm_dequeue_buf TYPE1 succeed. \n");	

	printk("[3A Porting]isp_k_rawafm_dequeue_buf: ret=0x%x\n", ret);
	return ret;
}
