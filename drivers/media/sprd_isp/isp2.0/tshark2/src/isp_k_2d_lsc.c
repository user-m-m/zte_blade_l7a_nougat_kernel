...
static int32_t isp_k_2d_lsc_param_load(struct isp_k_private *isp_private)
{
	...
	reg_value = REG_RD(ISP_LENS_PARAM);
	isp_private->lsc_load_buf_id = !((reg_value >> 1) & 1);
		...
		struct isp_k_private *isp_private)
{
	...
	spin_lock_irqsave(&isp_private->isr_lock, flag);
	if (isp_private->lsc_updated) {
		spin_unlock_irqrestore(&isp_private->isr_lock, flag);
		ret = 0;
		goto exit;
	}
	spin_unlock_irqrestore(&isp_private->isr_lock, flag);
