...
struct isp_k_private {
	...
	struct semaphore  rawae_schedule_sem;
		/* [Kernel_1/23] */
   struct isp_buf_queue af_type1_irq_queue; /*for irq read*/
   struct isp_buf_queue af_type2_irq_queue; /*for irq read*/
   struct isp_buf_queue af_type1_user_queue; /*for user read*/
   struct isp_buf_queue af_type2_user_queue; /*for user read*/
   struct isp_buf_node    af_reserved_node;
   uint32_t is_af_reserved_buf;
   struct semaphore         rawaf_type1_thread_sem;
   struct semaphore         rawaf_type2_thread_sem;
   struct task_struct      *rawaf_type1_thread;
   struct task_struct      *rawaf_type2_thread;
   uint32_t                 is_rawaf_type1_thread_stop;
   uint32_t                 is_rawaf_type2_thread_stop;
   struct isp_buf_node    rawaf_type1_cur_node;
   struct isp_buf_node    rawaf_type2_cur_node;
   uint32_t is_rawaf_type1_schedule;
   uint32_t is_rawaf_type2_schedule;
   struct semaphore  rawaf_type1_schedule_sem;
   struct semaphore  rawaf_type2_schedule_sem;
	uint32_t is_vst_ivst_update;
	unsigned int lsc_updated;
	spinlock_t isr_lock;
};
...
int isp_k_af_create_bq_thread(struct isp_k_private *isp_private);
int isp_k_stop_bq_thread(struct isp_k_private *isp_private);
int isp_k_af_stop_bq_thread(struct isp_k_private *isp_private);
