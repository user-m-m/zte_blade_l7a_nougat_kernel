...
static irqreturn_t isp_isr(int irq, void *dev_id)
{
	...
	spin_lock_irqsave(&s_isp_private->isr_lock, flag);
	memset(&node, 0x00, sizeof(node));
	ret = isp_get_int_num(&node);

	if (node.irq_val1 & BIT_22)
		s_isp_private->lsc_updated = 0;
	...
	spin_unlock_irqrestore(&s_isp_private->isr_lock, flag);
...
static int isp_open (struct inode *node, struct file *file)
{
	...
	spin_lock_init(&s_isp_private->isr_lock);
...
/* [Kernel_21/23] */
	ret = isp_k_af_create_bq_thread(isp_private);
	printk("[3A Porting]isp_open: isp_k_af_create_bq_thread.\n");
	if (unlikely(ret != 0)) {
		printk("isp_open: isp_k_af_create_bq_thread error.\n");
		ret = -EIO;
		goto open_exit;
	}
...
/* [Kernel_22/23] */
	isp_k_af_stop_bq_thread(isp_private);
	printk("[3A Porting]isp_stop: after isp_k_af_stop_bq_thread.\n");
